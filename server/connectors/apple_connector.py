"""
Apple iCloud Connector - IMAP/SMTP authentication
Location: server/connectors/apple_connector.py
"""
import logging
...
logger = logging.getLogger("apple_connector")


class AppleConnector:
    """Apple iCloud email connector using IMAP/SMTP"""
    ...
    def connect(self) -> Tuple[bool, str]:
        """
        Connect to iCloud email using IMAP
        ...
        try:
            ...
            if metadata.get("provider") != "apple":
                return False, "Account is not an Apple iCloud account"
            
            self.email_address = metadata.get("email")
            
            # Get app-specific password from vault
            self.app_password = self.vault.get_credentials(self.account_id, "app_password")
            if not self.app_password:
                return False, "No app-specific password found. Generate one at appleid.apple.com"
            
            # Connect to IMAP
            self.imap = IMAPClient(self.IMAP_SERVER, port=self.IMAP_PORT, ssl=True)
            self.imap.login(self.email_address, self.app_password)
            
            logger.info(f"Connected to iCloud: {self.email_address}")
            return True, f"Connected to {self.email_address}"
            
        except Exception as e:
            logger.error(f"Apple connection error: {e}")
            return False, str(e)
            ...
            for msg_id in msg_ids:
                try:
                    if permanent:
                        ...
                        self.imap.move([msg_id], 'Deleted Messages')
    ...
    def move_to_folder(self, email_ids: List[str], folder_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ensure folder exists (iCloud may have different folder structure)
            folders = [f[2] for f in self.imap.list_folders()]
